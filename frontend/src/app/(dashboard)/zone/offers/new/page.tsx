...
export default function NewOfferPage() {
  ...
  // Initialize zone user's assigned zone on mount
  useEffect(() => {
    const initZone = async () => {
      try {
        ...
        const zoneFromArray = Array.isArray(userData?.zones) && userData.zones.length > 0 ? userData.zones[0] : null
        const zoneId = zoneFromArray?.id ?? userData?.zoneId
        const zoneName = zoneFromArray?.name ?? userData?.zone?.name ?? undefined
        ...
        <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Zone Selection - Locked to Zone User's Zone */}
