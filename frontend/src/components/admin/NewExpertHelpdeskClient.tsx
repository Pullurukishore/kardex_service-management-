...
import { ArrowLeft, User, Mail, Phone, Lock, Eye, EyeOff, Save, Zap } from 'lucide-react';
import Link from 'next/link';
import { apiClient } from '@/lib/api/client';
import { toast } from 'sonner';

export default function NewExpertHelpdeskClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((response as any).user || response.data) {
        setMessage({ type: 'success', text: 'Expert Helpdesk created successfully! Redirecting...' });
        toast.success('Expert Helpdesk created successfully!');
        
        router.refresh();
        router.push('/admin/manage-expert-helpdesk');
      } else {
        throw new Error(response.message || 'Failed to create expert');
      }
    } catch (error: any) {
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to create expert';
      setMessage({ 
        type: 'error', 
        text: errorMessage
      });
